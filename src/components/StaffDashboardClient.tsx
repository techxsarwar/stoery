...
export default function StaffDashboardClient({
                ...
                allReports.map((report) => (
                  <div key={report.id} className={`bg-white border-4 border-on-surface p-6 rounded-2xl flex flex-col gap-4 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all ${report.status !== 'PENDING' ? 'opacity-50' : ''}`}>
                    <div className="flex justify-between items-start">
                      ...
                      <span className={`text-[8px] font-black uppercase tracking-widest px-3 py-1 rounded-full border-2 ${report.status === 'PENDING' ? 'bg-primary text-on-surface border-on-surface' : 'bg-on-surface/5 text-on-surface/40 border-on-surface/10'
                        }`}>
                        {report.status}
                      </span>
                    </div>
                ...
                allLicenses.map((license) => (
                  <div key={license.id} className={`bg-white border-4 border-on-surface p-6 rounded-2xl flex flex-col gap-4 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all ${license.status !== 'PENDING' ? 'opacity-60' : ''}`}>
                    <div className="flex justify-between items-start">
                      ...
                      <span className={`text-[8px] font-black uppercase tracking-widest px-3 py-1 rounded-full border-2 ${license.status === 'APPROVED' ? 'bg-emerald-500 text-white border-emerald-600' :
                    ...
                    <div className="p-8 bg-on-surface/5 border-t-4 border-on-surface grid grid-cols-1 md:grid-cols-4 gap-8">
                      ...
                      <div className="flex items-center justify-end">
                        <button
                          ...
                          className={`px-6 py-2 rounded-xl font-black text-xs uppercase tracking-widest transition-all border-2 border-on-surface shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] ${profile.isVerified
  ...
  return (
    <div className="min-h-screen bg-surface text-on-surface flex overflow-hidden font-sans selection:bg-primary/30">
      ...
      {/* Aegis Sidebar */}
      <aside className={`fixed md:relative top-0 left-0 h-screen z-50 w-72 md:w-20 lg:w-64 bg-primary border-r-4 border-on-surface flex flex-col items-start py-8 transition-transform duration-300 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
        }`}>
        ...
        <nav className="flex-grow w-full px-4 flex flex-col gap-2">
          {tabs.map((item) => (
            <button
              ...
              className={`flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-200 group w-full ${activeTab === item.name
