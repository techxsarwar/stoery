...
import { MessageSquare, Trash2, EyeOff, User } from "lucide-react";
    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col gap-4">
        {comments.map((comment) => (
          <div key={comment.id} className="bg-surface/60 backdrop-blur-md border-2 border-on-surface/10 p-6 rounded-lg flex gap-6 items-start group hover:border-primary/30 transition-all duration-300">
             ...
             <div className="flex-grow">
                ...
                <div className="flex gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
                   ...
                   <button className="flex items-center gap-2 font-label font-black text-[10px] uppercase text-on-surface-variant hover:text-on-surface transition-all">
                      <EyeOff size={12} />
                      {comment.isShadowBanned ? "Lift Curse" : "Shadow Curse"}
                   </button>
                   <button className="flex items-center gap-2 font-label font-black text-[10px] uppercase text-error hover:tracking-widest transition-all">
                      <Trash2 size={12} />
                      Purge
                   </button>
                </div>
