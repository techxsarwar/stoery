...
export default function CareerApplicationClient({ jobs }: { jobs: { id: string, title: string }[] }) {
  const searchParams = useSearchParams();
  const roleFromUrl = searchParams.get("role") || "";
  
  // Attempt to match the URL param to a jobId, otherwise empty
  const initialJobId = jobs.find(j => j.title === roleFromUrl)?.id || "";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await submitApplication({
      ...
      coverLetter: formData.whySoulpad
    });
  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col">
      ...
      <main className="flex-grow pt-32 pb-24 px-6 md:px-12 max-w-4xl mx-auto w-full flex flex-col gap-12">
        ...
        {/* Step 1: Basic Info */}
        {step === 1 && (
          <form onSubmit={(e) => { e.preventDefault(); setStep(2); }} className="flex flex-col gap-8 bg-surface-container-high border-2 border-on-surface/10 p-8 md:p-12 rounded-3xl shadow-xl animate-in fade-in slide-in-from-bottom-8">
